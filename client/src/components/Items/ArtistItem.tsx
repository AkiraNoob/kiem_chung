...
const ArtistItem: React.FC<ArtistItemProps> = ({ data }) => {
  const handleOnClickEachArtist = () => {
    console.log(data.id);
  };

  return (
    <div
      onClick={handleOnClickEachArtist}
      className="relative group flex flex-col items-center justify-center rounded-md overflow-hidden gap-x-4 bg-neutral-400/5 cursor-pointer hover:bg-neutral-400/10 transition p-5"
    >
